...
class User(Base):
    ...
    country = Column(String(3), default="US")  # ISO country code
...
class ProjectRequest(Base):
    ...
    expected_completion_time = Column(String(100))
    budget_range = Column(String(100))
    file_uploads = Column(Text)  # JSON string of file paths
    contact_email = Column(String(255))
    telegram_handle = Column(String(100))
    status = Column(String(50), default="Pending Review")  # Pending Review, In Progress, Completed, Cancelled
    user_id = Column(String(36), ForeignKey("users.id"), nullable=True)  # Optional, for logged-in users
    assigned_developer_id = Column(String(36), ForeignKey("users.id"), nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class Product(Base):
    ...
    slug = Column(String(255), unique=True, index=True)  # SEO-friendly URL slug
    ...
    file_path = Column(String(500))  # Path to the digital file
    file_size = Column(Integer)  # File size in bytes
    download_count = Column(Integer, default=0)
    
    # Demo and test fields
    youtube_demo_link = Column(String(500))  # YouTube video URL for product demo
    test_download_link = Column(String(500))  # Link to test version of the product
    
    # Activation fields
    max_activations = Column(Integer, default=1)  # Maximum number of simultaneous terminal activations
    
    # Version tracking
    version = Column(String(20), default="1.0.0")  # Product version for updates
    
    # Platform field
    platform = Column(String(20), default="MT4")  # MT4, MT5, or TradingView
...
class Transaction(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    user_id = Column(String(36), ForeignKey("users.id"))
    paystack_reference = Column(String(255), unique=True, index=True)
    amount = Column(Float)
...
class OrderItem(Base):
    ...
    # MySQL-specific optimizations
    __table_args__ = (
        Index('idx_order_item_transaction', 'transaction_id'),
...
class DownloadToken(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    user_id = Column(String(36), ForeignKey("users.id"))
    product_id = Column(String(36), ForeignKey("products.id"), nullable=True)  # Nullable for zip files
    transaction_id = Column(String(36), ForeignKey("transactions.id"))
    token = Column(String(255), unique=True, index=True)
    is_single_use = Column(Boolean, default=True)
    is_used = Column(Boolean, default=False)
    expires_at = Column(DateTime)
    file_path = Column(String(500), nullable=True)  # Path to file (for zip downloads)
    ...
    # MySQL-specific optimizations
    __table_args__ = (
        Index('idx_download_token_user', 'user_id'),
        Index('idx_download_token_expires', 'expires_at'),
        Index('idx_download_token_is_used', 'is_used'),
        {'mysql_engine': 'InnoDB', 'mysql_charset': 'utf8mb4', 'mysql_collate': 'utf8mb4_unicode_ci'}
    )
...
class DownloadLog(Base):
    ...
    user_id = Column(String(36), ForeignKey("users.id"))
    product_id = Column(String(36), ForeignKey("products.id"))
    license_id = Column(String(36), ForeignKey("licenses.id"), nullable=True)  # For direct downloads
    download_token_id = Column(String(36), ForeignKey("download_tokens.id"), nullable=True)  # For legacy token downloads
    ...
    download_status = Column(String(50), default="success")  # "success" or "failed"
    ...
    __table_args__ = (
        ...
        Index('idx_download_log_product', 'product_id'),
        Index('idx_download_log_time', 'download_time'),
        Index('idx_download_log_status', 'download_status'),
        {'mysql_engine': 'InnoDB', 'mysql_charset': 'utf8mb4', 'mysql_collate': 'utf8mb4_unicode_ci'}
    )
...
class Review(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_review_is_approved', 'is_approved'),
        {'mysql_engine': 'InnoDB', 'mysql_charset': 'utf8mb4', 'mysql_collate': 'utf8mb4_unicode_ci'}
    )
...
class BlogPost(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_blog_is_featured', 'is_featured'),
        {'mysql_engine': 'InnoDB', 'mysql_charset': 'utf8mb4', 'mysql_collate': 'utf8mb4_unicode_ci'}
    )
...
class BlogLike(Base):
    ...
    blog_post_id = Column(String(36), ForeignKey("blog_posts.id"), nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow)
    ...
    __table_args__ = (
        UniqueConstraint('user_id', 'blog_post_id', name='unique_user_post_like'),
        Index('idx_blog_like_user', 'user_id'),
        Index('idx_blog_like_post', 'blog_post_id'),
        {'mysql_engine': 'InnoDB', 'mysql_charset': 'utf8mb4', 'mysql_collate': 'utf8mb4_unicode_ci'}
    )
...
class BlogComment(Base):
    ...
    user_id = Column(String(36), ForeignKey("users.id"), nullable=False)
    blog_post_id = Column(String(36), ForeignKey("blog_posts.id"), nullable=False)
    parent_comment_id = Column(String(36), ForeignKey("blog_comments.id"), nullable=True)  # For replies
    content = Column(Text, nullable=False)
    ...
    # MySQL-specific optimizations
    __table_args__ = (
        Index('idx_blog_comment_post', 'blog_post_id'),
        Index('idx_blog_comment_user', 'user_id'),
        Index('idx_blog_comment_parent', 'parent_comment_id'),
        Index('idx_blog_comment_approved', 'is_approved'),
        {'mysql_engine': 'InnoDB', 'mysql_charset': 'utf8mb4', 'mysql_collate': 'utf8mb4_unicode_ci'}
    )
...
class Notification(Base):
    ...
    type = Column(String(50), default="info")  # info, success, warning, error, order, payment, system
...
class ExchangeRate(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    from_currency = Column(String(3), nullable=False)  # USD
    to_currency = Column(String(3), nullable=False)    # NGN
    ...
    __table_args__ = (
        ...
        Index('idx_exchange_rate_from', 'from_currency'),
        Index('idx_exchange_rate_to', 'to_currency'),
...
class ProjectResponse(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    project_request_id = Column(String(36), ForeignKey("project_requests.id"), nullable=False)
    admin_id = Column(String(36), ForeignKey("users.id"), nullable=False)
    parent_response_id = Column(String(36), ForeignKey("project_responses.id"), nullable=True)  # For threading
    response_type = Column(String(50), default="quote")  # quote, clarification, update, completion, message, reply
    title = Column(String(255), nullable=False)
    message = Column(Text, nullable=False)
    ...
    # MySQL-specific optimizations
    __table_args__ = (
        Index('idx_project_response_request', 'project_request_id'),
        Index('idx_project_response_admin', 'admin_id'),
        Index('idx_project_response_type', 'response_type'),
        Index('idx_project_response_created_at', 'created_at'),
        {'mysql_engine': 'InnoDB', 'mysql_charset': 'utf8mb4', 'mysql_collate': 'utf8mb4_unicode_ci'}
    )
...
class ProjectInvoice(Base):
    ...
    file_path = Column(String(500), nullable=True)  # Path to uploaded PDF file
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    ...
    # MySQL-specific optimizations
    __table_args__ = (
        Index('idx_project_invoice_request', 'project_request_id'),
        Index('idx_project_invoice_status', 'status'),
        Index('idx_project_invoice_due_date', 'due_date'),
        Index('idx_project_invoice_created_at', 'created_at'),
        {'mysql_engine': 'InnoDB', 'mysql_charset': 'utf8mb4', 'mysql_collate': 'utf8mb4_unicode_ci'}
    )
...
class ProjectProgress(Base):
    ...
    project_request_id = Column(String(36), ForeignKey("project_requests.id"), nullable=False)
    admin_id = Column(String(36), ForeignKey("users.id"), nullable=False)
    stage = Column(String(100), nullable=False)  # planning, development, testing, review, completed
    title = Column(String(255), nullable=False)
    description = Column(Text, nullable=False)
    ...
    # MySQL-specific optimizations
    __table_args__ = (
        Index('idx_project_progress_request', 'project_request_id'),
        Index('idx_project_progress_admin', 'admin_id'),
        Index('idx_project_progress_stage', 'stage'),
        Index('idx_project_progress_percentage', 'percentage_complete'),
        {'mysql_engine': 'InnoDB', 'mysql_charset': 'utf8mb4', 'mysql_collate': 'utf8mb4_unicode_ci'}
    )
...
class License(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    license_id = Column(String(100), unique=True, index=True)  # User-friendly license ID
    user_id = Column(String(36), ForeignKey("users.id"))
    product_id = Column(String(36), ForeignKey("products.id"))
    transaction_id = Column(String(36), ForeignKey("transactions.id"))
    is_active = Column(Boolean, default=True)
    ...
    __table_args__ = (
        ...
        Index('idx_license_is_active', 'is_active'),
...
class UserProductActivation(Base):
    ...
    account_login = Column(String(100))  # MT4/MT5 account login
    account_server = Column(String(255))  # MT4/MT5 server name
    is_active = Column(Boolean, default=True)
    ...
    # MySQL-specific optimizations
    __table_args__ = (
        Index('idx_activation_license', 'license_id'),
        Index('idx_activation_account', 'account_login'),
        Index('idx_activation_server', 'account_server'),
        Index('idx_activation_is_active', 'is_active'),
        {'mysql_engine': 'InnoDB', 'mysql_charset': 'utf8mb4', 'mysql_collate': 'utf8mb4_unicode_ci'}
    )
